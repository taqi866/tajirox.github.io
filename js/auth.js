        function handleLoginSuccess(res, loginBtn) {
            ...
            if (res.success) {
                ...
                if (currentUser.role === 'super_admin') {
                    ...
                    // Update both sidebar navigation and mobile bottom navigation for super_admin
                    const sidebarNav = document.getElementById('sidebarNav');
                    if (sidebarNav) {
                        sidebarNav.innerHTML = `
                            <button onclick="showPage('admin-dashboard')" class="sidebar-link active w-full flex items-center gap-3 p-3 rounded-xl transition-all font-bold text-indigo-600" data-i18n="admin_shops_manage">
                                <i class="fas fa-store-alt opacity-80"></i> <span data-i18n="admin_shops_manage">${t('admin_shops_manage')}</span>
                            </button>
                        `;
                    }
                    ...
                    if (mobileBottomNav) {
                        mobileBottomNav.innerHTML = `
                            <button onclick="showPage('admin-dashboard')" class="bottom-nav-item flex flex-col items-center justify-center gap-0.5 text-indigo-600 font-black active-press transition-all duration-200 flex-1 py-1.5">
                                <i class="fas fa-store-alt text-lg"></i>
                                <span class="text-[9px]" data-i18n="admin_shops_manage">${t('admin_shops_manage')}</span>
                            </button>
                        `;
                    }

                    // Modify refresh button for super_admin
                    const refreshBtn = document.querySelector('button[onclick="refreshData()"]');
                    if (refreshBtn) {
                        refreshBtn.setAttribute('onclick', 'loadAdminData()');
                    }

                    // Hide store-specific header alert buttons
                    const lowStockBtn = document.getElementById('lowStockAlertBtn');
                    if (lowStockBtn) lowStockBtn.style.setProperty('display', 'none', 'important');
                    const checksBtn = document.getElementById('checksAlertBtn');
                    if (checksBtn) checksBtn.style.setProperty('display', 'none', 'important');
