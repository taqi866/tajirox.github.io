        ...
        function openInvoiceModal() {
            ...
            document.getElementById('iDiscount').value = ''; // Ensure discount is cleared
            renderCart();
            ...
            if (cart.length === 0) {
                list.innerHTML = `
                    <li class="text-center p-4 text-slate-400 text-xs">
                        ...
                        <p class="text-slate-300 text-[10px] mt-1">${t('use_search_hint')}</p>
                    </li>
                `;
            } else {
                cart.forEach((it, idx) => {
        ...
        function confirmInvoice() {
            ...
            // Find client to get ICE & Address
            const selectedClient = custSelect.value ? allData.clients.find(c => c.id == custSelect.value) : null;
            const customerIce = selectedClient ? (selectedClient.ice || '') : '';
            const customerAddress = selectedClient ? (selectedClient.address || '') : '';
            ...
            const inv = {
                ...
                customer: selectedClient ? selectedClient.name : (custInput.value.trim() || t('general_customer')),
                customer_id: custSelect.value || '',
                customer_ice: customerIce,
            ...
            // 3. Mettre à jour le stock (pour les nouvelles factures uniquement ou si des items ont changé dans une facture éditée)
            if (existingInvoiceIndex === -1 && inv.type === 'بيع') {
            ...
            } else if (existingInvoiceIndex !== -1 && inv.type === 'بيع') {
                // Si c'est une édition, nous devons calculer la différence de stock
                const oldInv = allData.invoices[existingInvoiceIndex];
                ...
                const oldItemsMap = new Map(oldItems.map(item => [item.id, item.selectedQty]));
                const newItemsMap = new Map(newItems.map(item => [item.id, item.selectedQty]));

                // Parcourir tous les produits affectés (anciens et nouveaux)
                const allProductIds = new Set([...oldItemsMap.keys(), ...newItemsMap.keys()]);
                allProductIds.forEach(productId => {
                    const oldQty = oldItemsMap.get(productId) || 0;
                    const newQty = newItemsMap.get(productId) || 0;
                    const diff = newQty - oldQty; // Si diff > 0, stock diminue; si diff < 0, stock augmente
                    updateProductStockLocally(productId, -diff); // Appliquer l'inverse de la différence
                });
            }
            ...
            showToast(t('invoice_saved_locally'), 'success'); // [REQ] la facture doit être enregistrée avec succès
            ...
            if (currentUser && currentUser.invoiceSize === 'Thermal') {
                ...
                } else { // Fallback to standard browser print if QZ Tray is not running/connected
            ...
            // [REQ] la fenêtre « Créer une facture » ne doit pas se fermer ;
            // [REQ] une nouvelle facture vide doit être créée automatiquement ;
            // [REQ] tous les champs de la facture doivent être réinitialisés ;
            // [REQ] le focus doit être replacé automatiquement dans la barre de recherche « Chercher un produit », prête à recevoir immédiatement un nouveau scan.
            openInvoiceModal(); // This function already handles resetting all fields and preparing for a new invoice.
            setTimeout(() => { // Set focus to the search bar
                document.getElementById('iItemSearch').focus();
            }, 100);
    ...
    @media print {
        ...
        * {
        ...
        function generateTicketHTML(invoiceData, size, thermalWidth, forceDesign = null) {
            ...
            // Fetch ICE and Address with fallback for older invoices
            let customerIce = invoiceData.customer_ice || '';
            let customerAddress = invoiceData.customer_address || '';
            if ((!customerIce || !customerAddress) && invoiceData.customer_id && allData.clients) {
                const client = allData.clients.find(c => c.id == invoiceData.customer_id);
                if (client) {
                    if (!customerIce) customerIce = client.ice || '';
                    if (!customerAddress) customerAddress = client.address || '';
                }
            }
    ...
    `;
        ...
        `;
                ...
                <style>
                    ...
                    /* Custom color overrides for the modern header section (both on screen and print) */
                    .invoice-box .header-section {
                    ...
                    .invoice-box .header-section .shop-name,
                    .invoice-box .header-section .shop-details,
                    .invoice-box .header-section .invoice-title {
                        color: #ffffff !important;
                    }
                    ...
                    <div class="info-section">
                        ...
                        <div class="info-block">
                            ...
                            <div class="info-value">
                                ${invoiceData.customer || t('general_customer')}
                                ${customerIce ? `<br><span style="font-size:11px; font-weight:normal; color:#64748b;">${t('ice_label') || 'رقم ICE'}: ${customerIce}</span>` : ''}
                                ${customerAddress ? `<br><span style="font-size:11px; font-weight:normal; color:#64748b;">${t('address_label') || 'العنوان'}: ${customerAddress}</span>` : ''}
                            </div>
                        </div>
                        <div class="info-block">
            ...
            `;
                ...
                <table class="info-table">
                    <tr>
                        ...
                        <td width="50%" style="vertical-align: top;"><strong>${t('payment_method')}:</strong> ${paymentMethodText}</td>
                    </tr>
                </table>
                ...
                <div class="invoice-wrapper">
                    ...
                    <div class="meta-pro">
                        <div class="bill-to">
                            ...
                            ${customerIce ? `<br><span style="font-size:11px; color:#555;"><strong>${t('ice_label') || 'رقم ICE'}:</strong> ${customerIce}</span>` : ''}
                            ${customerAddress ? `<br><span style="font-size:11px; color:#555;"><strong>${t('address_label') || 'العنوان'}:</strong> ${customerAddress}</span>` : ''}
                        </div>
                ...
                <div class="invoice-creative">
                    ...
                    <div class="main-creative">
                        <div class="header-creative">
                            ...
                            <div>
                                <strong>${t('customer')}:</strong> ${invoiceData.customer || t('general_customer')}
                                ${customerIce ? `<br><small style="color:#666;"><strong>${t('ice_label') || 'رقم ICE'}:</strong> ${customerIce}</small>` : ''}
                                ${customerAddress ? `<br><small style="color:#666;"><strong>${t('address_label') || 'العنوان'}:</strong> ${customerAddress}</small>` : ''}
                            </div>
                        </div>
                        <table><thead><tr><th class="text-start">${t('product_col')}</th><th>${t('qty_label')}</th><th>${t('price_col')}</th><th style="text-align: ${isRtl ? 'left' : 'right'}">${t('total_label')}</th></tr></thead><tbody>${itemsHTML}</tbody></table>
            ...
            `;
                ...
                <div>
                    ...
                    <div class="meta-minimalist">
                        <div>
                            <strong>${t('customer')}:</strong><br>${invoiceData.customer || t('general_customer')}
                            ${customerIce ? `<br><small>${t('ice_label') || 'رقم ICE'}: ${customerIce}</small>` : ''}
                            ${customerAddress ? `<br><small>${t('address_label') || 'العنوان'}: ${customerAddress}</small>` : ''}
                        </div>
            ...
            `;
                ...
                <div class="invoice-details">
                    ...
                    <div style="text-align: ${isRtl ? 'left' : 'right'}">
                        ...
                        ${customerIce ? `<span style="color: #64748b; margin-top: 2px; display: inline-block;">${t('ice_label') || 'رقم ICE'}: ${customerIce}</span><br>` : ''}
                        ${customerAddress ? `<span style="color: #64748b; margin-top: 2px; display: inline-block;">${t('address_label') || 'العنوان'}: ${customerAddress}</span><br>` : ''}
        ...
        `;
            ...
            if (typeof window.invoicesLimit === 'undefined') {
                window.invoicesLimit = (typeof lowResourceMode !== 'undefined' && lowResourceMode) ? 10 : 30;
            }
            const limit = q ? ((typeof lowResourceMode !== 'undefined' && lowResourceMode) ? 30 : 100) : window.invoicesLimit;
...
`;
            ...
            if (!q && filteredInvs.length > limit) {
                container.innerHTML += `
                <div class="text-center py-4 bg-white rounded-3xl shadow-sm mt-2 flex flex-col items-center justify-center gap-2">
                    <p class="text-slate-400 text-xs font-bold">${t('search_results_stats', { filtered: limit, total: filteredInvs.length })}</p>
                    <button onclick="increaseInvoicesLimit()" class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs shadow-md transition-all active:scale-95">
                        ${currentLang === 'ar' ? 'عرض المزيد فواتير' : 'Charger plus de factures'}
                    </button>
                    <p class="text-slate-300 text-[10px] mt-1">${t('use_search_hint')}</p>
                </div>`;
            }
        ...
        window.increaseInvoicesLimit = function() {
            window.invoicesLimit = (window.invoicesLimit || 30) + ((typeof lowResourceMode !== 'undefined' && lowResourceMode) ? 10 : 30);
            renderInvoices(getFilteredData(allData.invoices));
        };
        ...
        function editInvoice(id) {
            ...
            document.getElementById('searchResults').innerHTML = '';
            document.getElementById('searchResults').classList.add('hidden');
            document.getElementById('iItemSearch').focus(); // Auto-focus for continuous scanning
        ...
        function saveServiceInvoice() {
            ...
            if (customerSelect.value) {
                ...
                customerIce = customer ? (customer.ice || '') : '';
