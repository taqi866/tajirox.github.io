    ...
    `;
        ...
        `;
            ...
            `;
                    ...
                    customCSS += `
                <style>
                    .invoice-vintage { border: 4px double var(--main-color); padding: 30px; position: relative; background: #fffdf9; color: #2c2c2c; }
                    .header-vintage { text-align: center; border-bottom: 2px dashed var(--main-color); padding-bottom: 20px; margin-bottom: 20px; }
                    .shop-name-vintage { font-size: 26px; font-weight: bold; font-family: serif; color: var(--main-color); text-transform: uppercase; text-shadow: 1px 1px 2px rgba(0,0,0,0.1); }
                    .shop-details { font-family: serif; font-style: italic; font-size: 12px; margin-top: 5px; color: #555; }
                    .meta-vintage { display: flex; justify-content: space-between; margin-bottom: 20px; font-size: 13px; border-bottom: 1px dashed var(--main-color); padding-bottom: 15px; }
                    table.vintage-table th { background: transparent; border: 1px solid var(--main-color) !important; color: var(--main-color); font-weight: bold; border-top: 3px double var(--main-color) !important; border-bottom: 3px double var(--main-color) !important; padding: 10px; }
                    table.vintage-table td { padding: 10px; border: 1px solid var(--main-color) !important; font-family: serif; }
                    .totals-vintage { float: ${isRtl ? 'left' : 'right'}; width: 45%; margin-top: 20px; border: 1px solid var(--main-color); padding: 15px; background: rgba(0,0,0,0.02); }
                    .total-row { display: flex; justify-content: space-between; padding: 5px 0; border-bottom: 1px dotted #ccc; font-family: serif; }
                    .total-row.final { font-weight: bold; font-size: 16px; color: var(--main-color); border-top: 2px solid var(--main-color); border-bottom: none; padding-top: 8px; }
                    .stamp-vintage { position: absolute; bottom: 80px; left: ${isRtl ? '40px' : 'auto'}; right: ${isRtl ? 'auto' : '40px'}; width: 90px; height: 90px; border: 3px double ${balance === 0 ? '#10b981' : '#f59e0b'}; border-radius: 50%; display: flex; align-items: center; justify-content: center; transform: rotate(-15deg); font-weight: 900; text-transform: uppercase; font-size: 12px; color: ${balance === 0 ? '#10b981' : '#f59e0b'}; opacity: 0.75; }
                </style>
            `;
                    templateHTML = `
                <div class="invoice-vintage">
                    <div class="header-vintage">
                        ${shopLogo ? `<img src="${shopLogo}" style="max-height: 70px; filter: grayscale(1); margin-bottom: 10px;">` : ''}
                        <div class="shop-name-vintage">${shopName}</div>
                        <div class="shop-details">${shopAddress} ${shopPhone ? ' — ' + shopPhone : ''}</div>
                    </div>
                    <div class="meta-vintage">
                        <div>
                            <strong>${t('customer')}:</strong> ${invoiceData.customer || t('general_customer')}<br>
                            <strong>${t('payment_method')}:</strong> ${paymentMethodText}
                        </div>
                        <div style="text-align: ${isRtl ? 'left' : 'right'};">
                            <strong>${t('invoice')} #:</strong> ${invoiceData.id}<br>
                    ...
                    <table class="vintage-table">
                        <thead>
                            <tr>
                                ...
                                <th width="15%">${t('qty_label')}</th>
                                <th width="20%">${t('price_col')}</th>
                                <th width="25%" style="text-align: ${isRtl ? 'left' : 'right'}">${t('total_label')}</th>
                            </tr>
                        </thead>
                        <tbody>${itemsHTML}</tbody>
                    </table>
                    ${paymentsHTML}
                    <div class="totals-vintage">
                        <div class="total-row"><span>${t('total')}:</span><span>${formatCurrency(total)}</span></div>
                        ${discount > 0 ? `<div class="total-row"><span>${t('discount')}:</span><span>-${formatCurrency(discount)}</span></div>` : ''}
                        <div class="total-row"><span>${t('paid')}:</span><span>${formatCurrency(paid)}</span></div>
                        <div class="total-row final"><span>${t('balance')}:</span><span>${formatCurrency(balance)}</span></div>
                    </div>
                    <div class="stamp-vintage">${balance === 0 ? (isRtl ? 'مدفوعة' : 'PAYÉ') : (isRtl ? 'غير كاملة' : 'DUE')}</div>
                    <div style="clear:both;"></div>
                    <div class="footer">${footerMessage}</div>
                </div>
            `;
                    ...
                    customCSS += `
                <style>
                    .invoice-geometric { border: 2px solid #111; padding: 0; background: #fff; font-family: sans-serif; }
                    .banner-geometric { background: var(--main-color); height: 15px; width: 100%; }
                    .header-geometric { display: flex; justify-content: space-between; align-items: center; padding: 30px; border-bottom: 2px solid #111; }
                    .shop-name-geometric { font-size: 24px; font-weight: 800; color: #111; letter-spacing: -1px; text-transform: uppercase; }
                    .meta-geometric { display: flex; justify-content: space-between; padding: 25px 30px; background: #f8fafc; border-bottom: 2px solid #111; }
                    .meta-box { width: 45%; }
                    .meta-box h4 { margin: 0 0 10px 0; font-weight: 800; color: var(--main-color); font-size: 13px; text-transform: uppercase; }
                    table.geo-table { margin: 0; width: 100%; border-collapse: collapse; }
                    table.geo-table th { background: #111; color: #fff; font-weight: 800; padding: 12px 15px; border: 1px solid #111 !important; text-transform: uppercase; }
                    table.geo-table td { padding: 12px 15px; border: 1px solid #111 !important; text-align: center; }
                    .totals-geo { display: flex; justify-content: flex-end; padding: 30px; }
                    .totals-box { width: 300px; border: 2px solid #111; padding: 15px; }
                    .total-row { display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px solid #eee; font-size: 13px; }
                    .total-row.final { font-weight: 800; font-size: 16px; border-top: 2px solid #111; border-bottom: none; padding-top: 10px; margin-top: 5px; color: var(--main-color); }
                </style>
            `;
                    templateHTML = `
                <div class="invoice-geometric">
                    <div class="banner-geometric"></div>
                    <div class="header-geometric">
                        <div>
                            <div class="shop-name-geometric">${shopName}</div>
                            <div style="font-size: 12px; color: #666; margin-top: 5px;">${shopAddress} | ${shopPhone}</div>
                        </div>
                        <div style="text-align: ${isRtl ? 'left' : 'right'};">
                            <h2 style="font-size: 32px; font-weight: 900; margin: 0; text-transform: uppercase; color: var(--main-color);">${t('invoice')}</h2>
                            <div style="font-size: 14px; font-weight: bold; margin-top: 5px;">#${invoiceData.id}</div>
                        </div>
                    </div>
                    <div class="meta-geometric">
                        <div class="meta-box">
                            <h4>${t('customer')}</h4>
                            <div><strong>${invoiceData.customer || t('general_customer')}</strong></div>
                            <div>${t('payment_method')}: ${paymentMethodText}</div>
                        </div>
                        <div class="meta-box" style="text-align: ${isRtl ? 'left' : 'right'};">
                            <h4>${t('date')}</h4>
                    ...
                    <div class="footer" style="padding: 20px 30px; border-top: 1px solid #eee;">${footerMessage}</div>
                </div>
            `;
                } else if (design === 'dashboard') {
                    customCSS += `
                <style>
                    .invoice-dashboard { border: 1px solid #e2e8f0; border-radius: 16px; padding: 30px; background: #ffffff; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); }
                    .header-dashboard { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #f1f5f9; padding-bottom: 25px; margin-bottom: 25px; }
                    .shop-badge { background: var(--main-color); color: #fff; padding: 6px 12px; border-radius: 8px; font-weight: bold; font-size: 11px; text-transform: uppercase; }
                    .shop-name-dash { font-size: 22px; font-weight: 800; color: #0f172a; margin-top: 8px; }
                    .grid-dashboard { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 20px; margin-bottom: 30px; }
                    .dash-card { background: #f8fafc; border-left: 4px solid var(--main-color); border-radius: 12px; padding: 15px; }
                    .dash-card-title { font-size: 10px; font-weight: 800; color: #64748b; text-transform: uppercase; margin-bottom: 5px; }
                    .dash-card-value { font-size: 14px; font-weight: bold; color: #0f172a; }
                    table.dash-table th { background: #f8fafc; color: #475569; border: 1px solid #e2e8f0 !important; font-weight: bold; padding: 12px 10px; }
                    table.dash-table td { border: 1px solid #e2e8f0 !important; padding: 12px 10px; color: #334155; }
                    .totals-dash { float: ${isRtl ? 'left' : 'right'}; width: 45%; margin-top: 20px; border-radius: 12px; border: 1px solid #e2e8f0; padding: 20px; background: #f8fafc; }
                    .total-row { display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px dashed #e2e8f0; font-size: 13px; }
                    .total-row.final { font-weight: 800; font-size: 16px; border-top: 2px solid var(--main-color); border-bottom: none; padding-top: 10px; color: var(--main-color); }
                </style>
            `;
                    templateHTML = `
                <div class="invoice-dashboard">
                    <div class="header-dashboard">
                        <div>
                            <span class="shop-badge">${t('invoice')}</span>
                            <div class="shop-name-dash">${shopName}</div>
                            <div style="font-size: 12px; color: #64748b; margin-top: 5px;">${shopAddress} | ${shopPhone}</div>
                        </div>
                        <div style="text-align: ${isRtl ? 'left' : 'right'};">
                            ${shopLogo ? `<img src="${shopLogo}" style="max-height: 50px; border-radius: 8px;">` : ''}
                            <div style="font-size: 14px; font-weight: 800; color: #0f172a; margin-top: 10px;">ID: #${invoiceData.id}</div>
                        </div>
                    </div>
                    <div class="grid-dashboard">
                        <div class="dash-card">
                            <div class="dash-card-title">${t('customer')}</div>
                            <div class="dash-card-value">${invoiceData.customer || t('general_customer')}</div>
                            <div style="font-size: 11px; color: #64748b; margin-top: 4px;">Method: ${paymentMethodText}</div>
                        </div>
                        <div class="dash-card" style="border-left-color: #3b82f6;">
                            <div class="dash-card-title">${t('date')}</div>
                            <div class="dash-card-value">${invoiceData.date}</div>
                        </div>
                    </div>
                    ...
                    <div style="clear:both;"></div>
                    <div class="footer" style="border:none; margin-top:30px; text-align:center; color:#94a3b8; font-size:11px;">${footerMessage}</div>
                </div>
            `;
                } else {
                    customCSS += `
