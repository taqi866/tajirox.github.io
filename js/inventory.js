        ...
        function searchInventory() {
            ...
            }, 300); // انتظار 300 ميلي ثانية بعد التوقف عن الكتابة
        }
    ...
    openConfirm({
        ...
        onConfirm: (val) => {
            ...
            for (let i = 0; i < copies; i++) {
                labelsHTML += `
                    <div class="thermal-barcode-label" style="width: ${bWidth}mm; min-height: ${bHeight}mm; padding: 4mm 2mm; margin: 0 auto; box-sizing: border-box; text-align: center; page-break-after: always;">
                        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%; height: 100%;">
                            ${shopLogo ? `<img src="${shopLogo}" style="max-height: 8mm; max-width: 90%; object-fit: contain; margin-bottom: 2mm;">` : `<div style="font-size: 3mm; font-weight: bold; margin-bottom: 2mm;">${shopName}</div>`}
                            <div style="font-size: 3.5mm; font-weight: bold; margin: 1mm 0; text-align: center; word-break: break-word;">${name}</div>
                            <svg class="barcode-svg" id="barcode-single-${i}" style="width: 100% !important; height: auto !important;"></svg>
<div style="font-size: 4mm; font-weight: bold; margin-top: 2mm; color: #333;">${id}</div>
                        </div>
                    </div>
                `;
            }
                ...
                onConfirm: (val) => {
                    ...
                    items.forEach(item => {
                        ...
                        <div class="thermal-barcode-label" style="width: ${bWidth}mm; min-height: ${bHeight}mm; padding: 4mm 2mm; margin: 0 auto; box-sizing: border-box; text-align: center; page-break-after: always;">
                            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%; height: 100%;">
                                ${shopLogo ? `<img src="${shopLogo}" style="max-height: 8mm; max-width: 90%; object-fit: contain; margin-bottom: 2mm;">` : `<div style="font-size: 3mm; font-weight: bold; margin-bottom: 2mm;">${shopName}</div>`}
                                <div style="font-size: 3.5mm; font-weight: bold; margin: 1mm 0; text-align: center; word-break: break-word;">${item.name}</div>
                                <svg class="barcode-svg" data-code="${item.id}" style="width: 100% !important; height: auto !important;"></svg>
<div style="font-size: 4mm; font-weight: bold; margin-top: 2mm; color: #333; letter-spacing: 0.5mm;">${item.id}</div>
                            </div>
                        </div>
                    `;
                            totalLabels++;
        ...
        <div class="inventory-card ${stockClass}">
            ...
            <div class="card-footer">
                <div class="flex gap-2 justify-end">
                    ${item.category !== 'خدمة' ? `
                        <button onclick="promptAddStock('${item.id}', '${item.name}')" 
