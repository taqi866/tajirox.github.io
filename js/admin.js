        ...
        function renderAdminStats(stats, shops) {
                    ...
                    return `
                        <div class="space-y-1">
                            <div class="flex justify-between text-[11px] font-black text-slate-700">
                                <span class="flex items-center gap-1.5"><i class="${icon} text-slate-400 text-[10px]"></i> ${name}</span>
                                <span>${count}</span>
                            </div>
            ...
            container.innerHTML = `
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <!-- Card 5: Countries count -->
                    <div class="bg-gradient-to-br from-amber-500 to-amber-600 p-5 rounded-[2rem] shadow-sm text-white flex items-center justify-between hover:scale-[1.02] transition-all">
                        <div>
                            <p class="text-[10px] font-bold opacity-80 mb-1">${t('stats_active_countries') || "الدول النشطة"}</p>
                            <h3 class="text-3xl font-black">${Object.keys(stats.countries || {}).length}</h3>
                        </div>
                        <div class="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center text-xl">
                    ...
                    <!-- Card 6: Cities count -->
                    <div class="bg-gradient-to-br from-rose-500 to-rose-600 p-5 rounded-[2rem] shadow-sm text-white flex items-center justify-between hover:scale-[1.02] transition-all">
                        <div>
                            <p class="text-[10px] font-bold opacity-80 mb-1">${t('stats_active_cities') || "المدن النشطة"}</p>
                            <h3 class="text-3xl font-black">${Object.keys(stats.cities || {}).length}</h3>
                        </div>
                        <div class="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center text-xl">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4 flex flex-col h-[300px]">
                        <h4 class="text-xs font-black text-slate-800 flex items-center gap-2 border-b pb-2 flex-shrink-0">
                            <i class="fas fa-flag text-indigo-600"></i> ${t('stats_top_countries') || "أعلى الدول تصفحاً"}
                        </h4>
                        <div class="space-y-3 overflow-y-auto flex-1 pr-1 modal-scrollable-content">
                            ${renderProgressBars(topCountries, 'fas fa-globe-africa')}
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4 flex flex-col h-[300px]">
                        <h4 class="text-xs font-black text-slate-800 flex items-center gap-2 border-b pb-2 flex-shrink-0">
                            <i class="fas fa-city text-emerald-600"></i> ${t('stats_top_cities') || "أعلى المدن تصفحاً"}
                        </h4>
                        <div class="space-y-3 overflow-y-auto flex-1 pr-1 modal-scrollable-content">
                            ${renderProgressBars(topCities, 'fas fa-map-pin')}
                        </div>
                    </div>
        ...
        function saveRenewal() {
            ...
            element.innerHTML = `
                <div style="font-family: 'Cairo', sans-serif; padding: 40px; background: #fff; direction: ${dir}; text-align: ${align}; border: 10px double #2563eb; width: 190mm; box-sizing: border-box; position: relative; margin: 0 auto;">
                    <style>
                    ...
                    <div style="margin-bottom: 30px; padding: 12px 18px; background-color: #fffbeb; border-radius: 12px; border: 1px solid #fde68a; color: #b45309; font-size: 11.5px; font-weight: bold; display: flex; align-items: center; line-height: 1.5; border-inline-start: 5px solid #f59e0b;">
                        <span style="font-size: 16px; margin-inline-end: 8px;">⚠️</span>
                        <span style="flex: 1;">${t('cert_renewal_warning')}</span>
                    </div>

                    <!-- Footer -->
                    <div style="text-align: center; margin-top: 20px; border-top: 2px solid #e2e8f0; padding-top: 15px;">
                    ...
                    ${includeStamp ? `<img src="stamp.png" style="position: absolute; bottom: 30px; ${dir === 'ar' ? 'left: 45px;' : 'right: 45px;'} width: 110px; height: 110px; z-index: 10;" alt="Stamp">` : ''}
                </div>
            `;
