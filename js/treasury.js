    ...
    if (criticalStock.length > 0) {
        ...
        function checkDueDateAlerts() {
            ...
            // عرض التنبيهات
            if (overdueChecks.length > 0) { // [REQ] The user wants to remove this specific message.
    ...
    filteredChecks.forEach(check => {
        ...
        function renderTreasury() {
            ...
            if (typeof window.treasuryLimit === 'undefined') {
                window.treasuryLimit = (typeof lowResourceMode !== 'undefined' && lowResourceMode) ? 10 : 30;
            }
            const limit = window.treasuryLimit;
            ...
            if (allMovements.length > limit) {
                container.innerHTML += `
                    <div class="text-center py-4 bg-white rounded-2xl shadow-sm mt-2 flex flex-col items-center justify-center gap-2">
                        <p class="text-slate-400 text-[10px] font-bold">${t('treasury_stats', { filtered: limit, total: allMovements.length })}</p>
                        <button onclick="increaseTreasuryLimit()" class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs shadow-md transition-all active:scale-95">
                            ${currentLang === 'ar' ? 'عرض المزيد عمليات' : 'Charger plus de transactions'}
                        </button>
        ...
        window.increaseTreasuryLimit = function() {
            window.treasuryLimit = (window.treasuryLimit || 30) + ((typeof lowResourceMode !== 'undefined' && lowResourceMode) ? 10 : 30);
            renderTreasury();
        };
